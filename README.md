...
- phase4：owner companion executor 已接入，支持 pairing、heartbeat、assignment、scope、structured logs 和 session event 回放。
...
- 自动化控制面：支持 Tasks、Task Runs、Triggers、Approvals、Executors、workflow templates 和 companion 执行器。
...
### 可选：本地配对 companion 执行器

如果你要测试 executor-backed workflows：

1. 打开 `http://localhost:3000/miniapp/`
2. 在 `Executors` 面板创建 executor 并点击 `Pair`
...
3. 阅读 [自动化控制面设计](doc/automation-control-plane.md) 理解 task runtime、approval、trigger 和 companion。
...
- task run 如果卡住，优先看 `Tasks` 的 workflow preview、`Sessions` 的事件流，以及 `Executors` 的在线状态和 companion logs。
