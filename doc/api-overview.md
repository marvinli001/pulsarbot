...
| `POST` | `/api/executors/:id/attach` | Chrome extension executor 显式 attach 当前浏览器窗口 |
| `POST` | `/api/executors/:id/detach` | Chrome extension executor 显式 detach 当前浏览器窗口 |
| `POST` | `/api/executors/:id/force-detach` | owner 强制解除 stale browser attachment |
| `POST` | `/api/executors/:id/heartbeat` | executor heartbeat、领取 assignment、回传完成结果 |
...
- `POST /api/executors/:id/heartbeat` 现在支持两类日志回传：
  - `executorLogs`: heartbeat 级别日志，例如 pairing、assignment batch、heartbeat 失败重试前的状态
